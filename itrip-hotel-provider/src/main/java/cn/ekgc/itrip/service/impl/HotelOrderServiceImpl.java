...
import cn.ekgc.itrip.util.DateUtil;
...
public class HotelOrderServiceImpl implements HorderOrderService {
	@Autowired
	...
	public RoomStoreVO getpreorderinfo(ValidateRoomStoreVO validateRoomStoreVO) throws Exception {
		...
		queryMap.put("hotelId",roomStoreVO.getHotelId());
		queryMap.put("roomId", roomStoreVO.getRoomId());
		queryMap.put("checkInDate", roomStoreVO.getCheckInDate());
		queryMap.put("checkOutDate",roomStoreVO.getCheckOutDate());
		// 查询临时库存表获得对应的库存信息
		...
		// 获得hotel信息
		Map<String,Object>hotelQueryMap =new HashMap<String, Object>();
		...
		//获得房间信息
		Map<String,Object>roomQueryMap=new HashMap<String, Object>();
	...
	public Map<String, Object> addHotelOrder(HotelOrderAddVO hotelOrderAddVO) throws Exception {
		...
		for (UserLinkUser userLinkUser:hotelOrderAddVO.getLinkUser()) {
			stringBuffer.append(userLinkUser.getLinkUserName()+",");
		}
		hotelOrder.setLinkUserName(stringBuffer.toString());
	...
	public PersonalOrderRoomVO getpersonalorderroominfo(Long orderId) throws Exception {
		...
		if (orderList != null && orderList.size() > 0) {
			...
			System.out.println(orderList.get(0).getHotelId());
	 ...
	 * @return
	 */
	public Page<HotelOrder> getpersonalorderlist(ItripSearchOrderVO vo, Long id) throws Exception {
